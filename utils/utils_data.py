...
def ordinal_tabular_preprocess(mem, non_mem, synth, ref, fit_target='synth'):
    """
    ...
    transformer = fit_transformer(fit_data)
...
def load_required_files(base_path, dataset, seed, method):
    """
    ...
    if seed_path.exists():
        ...
        if holdout_set is not None:
            ...
            split_point = len(holdout_set) // 2
            
            # Split into two parts
            dfs['non_mem'] = holdout_set_shuffled[:split_point]
            dfs['ref'] = holdout_set_shuffled[split_point:]
